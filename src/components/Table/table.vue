<template>
    <div class="k-table-wrapper" ref="wrapper">
        <div class="k-table-wrapper2" ref="tableWrapper">
            <table class="k-table" :class="{bordered, striped, [`${size}`]: true}" ref="table">
                <thead ref="tableHeader">
                <tr>
                    <th class="k-table-input-th" v-if="selectedItems">
                        <div class="k-table-input-wrapper" :class="{checked: selectAll, indeterminate: indeterminate}"
                             @click="onClickAll"></div>
                    </th>
                    <th v-for="column in columns" v-if="columns.length > 0" :key="column.field" :style="{width: column.width+'px'}">
                        <div class="k-table-field-wrapper">
                            <div>{{column.name}}</div>
                            <div class="icon-group" @click="changeSortRule(column.field)"
                                 v-if="sortDirections && sortDirections[column.field]">
                                <k-icon name="up-solid"
                                        :class="{active: sortDirections[column.field] ==='asc'}"></k-icon>
                                <k-icon name="down-solid"
                                        :class="{active: sortDirections[column.field] ==='desc'}"></k-icon>
                            </div>
                        </div>
                    </th>
                ...
                <tr v-for="source in dataSource" :key="source.id">
                    <td class="k-table-input-td" v-if="selectedItems" style="">
                        <div class="k-table-input-wrapper" :class="{checked: isChecked(source.id)}"
        ...
        <div v-if="loading" class="k-table-loading">
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.columns = this.$slots['k-column'].map(vNode => {
                let width, render
        ...
        methods: {
            fixedHeader() {
                ...
                tableCopy.classList.add('k-table-copy')
...
<style scoped lang="scss">
    ...
    .k-table-wrapper {
        ...
        .k-table-input-th, .k-table-input-td {
            width: 20px;
        }
